...
if(!state.v13?.relationshipArcs?.['ally-a']||state.v13.relationshipArcs['ally-a'].loyalty!==state.v19.records['ally-a'].axes.loyalty)failures.push('legacy V13 loyalty arc is not synchronized');
...
const bootstrap=fs.readFileSync(path.join(root,'js','bootstrap.js'),'utf8'),sw=fs.readFileSync(path.join(root,'sw.js'),'utf8'),css=fs.readFileSync(path.join(root,'styles','v19.css'),'utf8'),experience=fs.readFileSync(path.join(root,'js','v19-experience.js'),'utf8'),hardening=fs.readFileSync(path.join(root,'js','v19-hardening.js'),'utf8'),pkg=JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8'));
for(const ref of ['styles/v19.css','js/domain/v19-engine.js','js/v19-experience.js','js/v19-hardening.js'])if(!bootstrap.includes(ref))failures.push(`bootstrap does not load ${ref}`);
for(const ref of ['styles/v19.css','js/domain/v19-engine.js','js/v19-experience.js','js/v19-hardening.js'])if(!sw.includes(ref))failures.push(`service worker does not cache ${ref}`);
...
for(const marker of ['assistDecision','syncLegacy','progressPersonalQuests','_v13ExplicitUpdateReload','openTitleV13'])if(!hardening.includes(marker))failures.push(`V19 hardening marker missing: ${marker}`);
...
const report={schema:state.v19.schemaVersion,axes:AXES.length,morale:summary.morale,pairCompatibility:pair.compatibility,personalQuests:Object.keys(summary.personalQuests).length,resonantAscension:engine.bondSurge(state,roster).ready,firstInstallClaimGuard:sw.includes('oldCaches.length'),failures};
if(process.argv.includes('--json'))console.log(JSON.stringify(report,null,2));else console.log(`V19 content valid: ${report.axes} relationship axes, morale ${report.morale}, pair compatibility ${report.pairCompatibility}, ${report.personalQuests} personal quests, Resonant Ascension ${report.resonantAscension?'ready':'blocked'}, first-install claim guard ${report.firstInstallClaimGuard?'on':'off'}.`);
