...
const cacheMajor=Number(sw.match(/multiverse-wheel-v(\d+)-/)?.[1]||0);if(cacheMajor<22)failures.push(`service-worker cache major V${cacheMajor||0} is older than V22`);
...
const report={schema:state.v22.schemaVersion,settlements:Object.keys(state.v22.settlements).length,needs:Object.keys(NEED_DEFS_V22).length,actions:Object.keys(CIVILIAN_ACTIONS_V22).length,sanctuaryBuilt:sanctuary.ok,marketModifier:engine.marketModifier(state),packageVersion:pkg.version,cacheMajor,firstInstallClaimGuard:sw.includes('oldCaches.length'),failures};
if(process.argv.includes('--json'))console.log(JSON.stringify(report,null,2));else console.log(`V22 content valid: ${report.settlements} settlements, ${report.needs} need families, ${report.actions} relief actions, sanctuary ${report.sanctuaryBuilt?'on':'off'}, market modifier ${Math.round(report.marketModifier*100)}%, service-worker cache V${report.cacheMajor}, first-install claim guard ${report.firstInstallClaimGuard?'on':'off'}.`);
