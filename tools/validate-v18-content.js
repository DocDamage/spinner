...
const pkg=JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8')),manifest=JSON.parse(fs.readFileSync(path.join(root,'manifest.webmanifest'),'utf8')),major=Number(String(pkg.version||'0').split('.')[0]),manifestMajor=Number((String(manifest.name||'').match(/V(\d+)/)||[])[1]||0);if(!Number.isFinite(major)||major<18)failures.push(`package major ${pkg.version} is older than V18`);if(manifestMajor<18)failures.push(`PWA manifest ${manifest.name} is older than V18`);
