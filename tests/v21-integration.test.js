...
test('Training Hall adds bounded bonus mastery through V20 gainGearMastery',()=>{const state=makeState(),{engine,hold}=setup(state),relics=new MultiverseDomain.RelicMasteryEngine(),record=relics.gearRecord(state,state.lootInventory[0]);record.xp=0;level(engine,state,hold,'training',3);relics.gainGearMastery(state,{type:'training',outcome:'win'});const current=state.v20.gear['gear-a'];assert.ok(current.xp>10);assert.ok(current.xp<=20);});

test('Relic Vault strengthens V20 purification while retaining the same relic state',()=>{const state=makeState(),{engine,hold}=setup(state),relics=new MultiverseDomain.RelicMasteryEngine();level(engine,state,hold,'vault',1);const relic=relics.relicRecord(state,'relic-a',artifacts[0]);relic.corruption=60;relic.purity=35;const result=relics.purify(state,'relic-a'),current=state.v20.relics['relic-a'];assert.equal(result.ok,true);assert.ok(result.v21VaultBonus>0);assert.equal(current.id,'relic-a');assert.equal(result.relic.id,current.id);assert.equal(result.relic.corruption,current.corruption);assert.equal(result.relic.purity,current.purity);assert.equal(Object.keys(state.v20.relics).filter(id=>id==='relic-a').length,1);assert.ok(current.corruption<30);assert.ok(current.purity>53);});
